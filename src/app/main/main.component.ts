...
export class MainComponent implements OnInit {
  ...
  constructor(
    ...
    this.maxCovDate.setDate(this.maxCovDate.getDate() - 2);
  }

  ngOnInit(): void {
    this.getCountries();
    this.covidService.twoDates('Spain', new Date('2020-2-22'), new Date('2020-2-24')).subscribe(
      data => {
        console.log(data);
      }
    );
  ...
  loadData(event: any): void {
    if (this.country && this.dateInit && this.dateEnd) {
      forkJoin([
        ...
        this.covidService.twoDates(this.country, this.dateInit, this.dateEnd).pipe(map(data => data.map(val => val.confirmed - val.recovered - val.deaths))),
